...
namespace jcPimSoftware
{
    class Pim_Sweep
    {
        ...
        protected void Exectue()
        {
            ...
            if (exe_params.SweepType == SweepType.Freq_Sweep)
            {
                try
                {
                    ...
                    IAsyncResult Ir_RF1_SetFirst = del_RF1_SetFirst.BeginInvoke(exe_params.DevInfo.RF_Addr1,
                                                                      exe_params.RFPriority, exe_params.FrqParam.Items1[0].P1, true, null, null);
                    IAsyncResult Ir_RF2_SetFirst = del_RF2_SetFirst.BeginInvoke(exe_params.DevInfo.RF_Addr2,
                                                                     exe_params.RFPriority, exe_params.FrqParam.Items1[0].P2, true, null, null);
            ...
            else
            {
                try
                {
                    ...
                    //设置功率，打开功放
                    IAsyncResult Ir_RF1_SetFirst = del_RF1_SetFirst.BeginInvoke(exe_params.DevInfo.RF_Addr1,
                                                                    exe_params.RFPriority, exe_params.TmeParam.P1, true,
                                                                    null, null);
                    IAsyncResult Ir_RF2_SetFirst = del_RF2_SetFirst.BeginInvoke(exe_params.DevInfo.RF_Addr2,
                                                                  exe_params.RFPriority, exe_params.TmeParam.P2, true,
                                                                  null, null);
       ...
       public  List<string> errmessage = new List<string>();
        ...
        private void FanShuangAddProcess(ref TimeSweepParam tsp)
        {
            ...
            if (App_Configure.Cnfgs.Spectrum == SpectrumType.FanShuang)
            {
                ...
                if (tsp != null)
                {
                    ...
                    //Log.WriteLog("start tx1=" + tsp.F1.ToString() + " tx2=" + tsp.F2.ToString() + " rx=" + tsp.Rx.ToString(), Log.EFunctionType.PIM);
                    freqMax = Math.Max(tsp.F1, tsp.F2);
                    freqMin = Math.Min(tsp.F1, tsp.F2);
                    if ((Math.Abs((freqMax - tsp.Rx)) >= 139.3 && Math.Abs((freqMax - tsp.Rx)) <= 140.7) || (Math.Abs((freqMin - tsp.Rx)) >= 139.3 && Math.Abs((freqMin - tsp.Rx)) <= 140.7) || (Math.Abs((freqMax - tsp.Rx)) == 70 || Math.Abs((freqMin - tsp.Rx)) == 70))
                    //if (freqMax - 140 == tsp.Rx || freqMin - 140 == tsp.Rx)
                    //if (freqMax - tsp.Rx <= 140.7 || freqMin - tsp.Rx >= 139.3)
                    {
                        if (tsp.F1 > tsp.F2)
                        {
                            tsp.F1 -= offset__;
                        ...
                        //tsp.Rx = ((imo - 1) / 2 + 1) * freqMin - ((imo - 1) / 2 * freqMax);
                        //Log.WriteLog("end tx1=" + tsp.F1.ToString() + " tx2=" + tsp.F2.ToString() + " rx=" + tsp.Rx.ToString(), Log.EFunctionType.PIM);
                    ...
                    else
                    {
                        ...
                        if (tsp.F1 > tsp.F2)
                        {
                            temporaryVariables = (int)tsp.F2;
                            if (temporaryVariables == tsp.F2)
                            {
                                tsp.F2 += 0.1f;
                                freqMin += 0.1f;
                            }
                        ...
                        {
                            temporaryVariables = (int)tsp.F1;
                            if (temporaryVariables == tsp.F1)
                            {
                                tsp.F1 += 0.1f;
                                freqMin += 0.1f;
                            }
        ...
        {
            //Log.WriteLog("\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\" , Log.EFunctionType.PIM);
            /*****************************************************************************
             * 凡双收信机中频为70MHz,由于其滤波器性能受限，会在其中频2倍频中引入一个谐波分量，
             * 这里采用频偏的方式将其避开。
             * nF1 - mF2 -> F2 -= 0.1             
            ...
            if (App_Configure.Cnfgs.Spectrum == SpectrumType.FanShuang)
            {
                ...
                if (fsi != null)
                {
                    ...
                    //if ((freqMax - fsi.Rx >= 139.3 && (freqMax - fsi.Rx <= 140.7)) || (freqMin - fsi.Rx >= 139.3 && (freqMin - fsi.Rx <= 140.7)) || (freqMax - fsi.Rx <= 72 || freqMin - fsi.Rx >= 72))
                    //if (freqMax - fsi.Rx <= 140.7  || freqMin - fsi.Rx >= 139.3)
                    if ((Math.Abs((freqMax - fsi.Rx)) >= 139.3 && Math.Abs((freqMax - fsi.Rx)) <= 140.7) || (Math.Abs((freqMin - fsi.Rx)) >= 139.3 && Math.Abs((freqMin - fsi.Rx)) <= 140.7) || (Math.Abs((freqMax - fsi.Rx)) == 70 || Math.Abs((freqMin - fsi.Rx)) == 70))
                    {
                        //if (n == 0)
                        ...
                        //Log.WriteLog("fsi.tx1=  " + fsi.Tx1.ToString() + "  fsi.tx2= " + fsi.Tx2.ToString(), Log.EFunctionType.PIM);
                        int imo = PimForm.pimOrder;
                    ...
                    else
                    {
                        ...
                        if (fsi.Tx1 > fsi.Tx2)
                        {
                            temporaryVariables = (int)fsi.Tx2;
                            if (temporaryVariables == fsi.Tx2)
                            {
                                freqMin += 0.1f;
                                fsi.Tx2 += 0.1f;
                            }
                        ...
                        {
                            temporaryVariables = (int)fsi.Tx1;
                            if (temporaryVariables == fsi.Tx1)
                            {
                                freqMin += 0.1f;
                                fsi.Tx1 += 0.1f;
                            }
                        ...
                        if (App_Configure.Cnfgs.Mode >= 2)
                        {
                            if (PimForm.port1_rev_fwd == 1 || PimForm.port1_rev_fwd == 2)
                            {
                                fsi.Rx = ((imo2 - 1) / 2 + 1) * freqMax - ((imo2 - 1) / 2 * freqMin);
                            }
                            else
                            {
                                fsi.Rx = ((imo2 - 1) / 2 + 1) * freqMin - ((imo2 - 1) / 2 * freqMax);
                            }
                        }
                        else
                        {
            ...
            //Log.WriteLog("FanShuangAddProcess  tx1=" + fsi.Tx1.ToString() + " tx2=" + fsi.Tx2.ToString() + " rx=" + fsi.Rx.ToString(), Log.EFunctionType.PIM);
        }
